...
class LoraLayer(BaseTunerLayer):
    ...
    other_param_names: tuple[str, ...] = ("r", "lora_alpha", "scaling", "lora_dropout", "lora_type", "lora_beta", "lora_gamma", "lora_delta")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        # For curvature
        self.lora_k = nn.ParameterDict({})
        self.lora_norm_scale = nn.ParameterDict({})
        self.lora_beta = {}
        self.lora_gamma = {}
        self.lora_delta = {}
        # For v4: learnable curvature bounds
    ...
    def update_layer(
        ...
        lora_type_parts = lora_type.split('-')
        self.lora_type[adapter_name] = lora_type_parts[0]

        if len(lora_type_parts) > 1:
            k = float(lora_type_parts[1])  # get the curvature
            if len(lora_type_parts) > 2:
                extra_value = float(lora_type_parts[2])  # method-specific scalar (e.g. beta/gamma)
            else:
                extra_value = 0.0
            if len(lora_type_parts) > 3:
                extra_value2 = float(lora_type_parts[3])  # optional second method-specific scalar (e.g. delta)
            else:
                extra_value2 = 0.0
        else:
            k = 1.0  # default curvature
            extra_value = 0.0  # default method-specific scalar
            extra_value2 = 0.0  # default second method-specific scalar
        ...
        if self.lora_type[adapter_name] == 'std':
            self.lora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)
            self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=lora_bias)
            self.lora_bias[adapter_name] = lora_bias
        
        elif self.lora_type[adapter_name] == 'hyplora':
            
            self.lora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)
            self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=False)
            self.lora_bias[adapter_name] = False
            self.lora_k[adapter_name] = nn.Parameter(torch.tensor(k), requires_grad=True)

            def clip_k_gradient(grad):
                return torch.clamp(grad, min=-0.01, max=0.01)

            self.lora_k[adapter_name].register_hook(clip_k_gradient)

            initial_scale = math.sqrt(self.in_features)
            self.lora_norm_scale[adapter_name] = nn.Parameter(
                torch.tensor(initial_scale), requires_grad=True
            )

        elif self.lora_type[adapter_name] == 'hyplora_simplified':
            # For HyperLoRA Simplified, we work in the hyperbolic space after projection
            self.lora_A[adapter_name]  = nn.Linear(self.in_features, r, bias=False)
            self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=False)
            self.lora_bias[adapter_name] = False
            self.lora_k[adapter_name] = nn.Parameter(torch.tensor(k), requires_grad=True)
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._norm_scale_minmax = {"max": 1.0*self.in_features ** 0.5, "min": 10.0}
    ...
    def hyplora_simplified(self, x, k):
        """
        HyperLoRA Simplified projection from hyperbolic to Euclidean space.
        Args:
            x: Input tensor with shape (..., n+1) where last dimension is [ξ; x]
            k: Curvature parameter (note: actual curvature is -1/k)
        Returns:
            Projected tensor with shape (..., n)
        """
        xi = x[..., 0]  # First component ξ
        x_space = x[..., 1:]  # Spatial components x
        denom = 1 + torch.sqrt(torch.abs(k)) * xi
        return x_space / denom.unsqueeze(-1)

    def hyplora_simplified_inverse(self, y, k):
        """
        Inverse HyperLoRA Simplified projection from Euclidean to hyperbolic space.
        Args:
            y: Input tensor with shape (..., n)
            k: Curvature parameter (note: actual curvature is -1/k)
        Returns:
            Tensor in hyperbolic space with shape (..., n+1)
        """
        y_norm_sq = torch.sum(y * y, dim=-1)
        xi = (1/torch.sqrt(torch.abs(k))) * ((1 - k * y_norm_sq) / (1 + k * y_norm_sq))
        y_scaled = (2 * y) / (1 + k * y_norm_sq.unsqueeze(-1))
        return torch.cat([xi.unsqueeze(-1), y_scaled], dim=-1)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA
                    lora_type = self.lora_type[active_adapter]
                    if lora_type == "std":
                        interim = lora_B(lora_A(dropout(x))) * scaling
                        result = result + interim
                    elif lora_type == "hyplora":
                        k = self.lora_k[active_adapter]
                        ...
                        with torch.cuda.amp.autocast(enabled=False):
                            ...
                            x_norm = torch.norm(x_fp32, p=2, dim=-1, keepdim=True).clamp(min=1e-7)
                            x_direction = x_fp32 / x_norm

                            k_positive = torch.nn.functional.softplus(k_fp32) + 1e-6
                            sqrtK = torch.sqrt(k_positive)

                            feature_scale = self.lora_norm_scale[active_adapter]
                            x_norm_scaled = x_norm / feature_scale

                            x_space = sqrtK * torch.sinh(torch.clamp(x_norm_scaled / sqrtK, max=12.0)) * x_direction
                            x_space = lora_A(x_space)
                            x_space = lora_B(x_space)

                            # Lorentz constraint with curvature-dependent normalization:
                            k_safe = torch.clamp(k_positive, min=1e-6)
                            x_time_new = ((x_space ** 2).sum(dim=-1, keepdim=True) + 1.0 / k_safe).sqrt()

                            x_space_norm = torch.norm(x_space, p=2, dim=-1, keepdim=True).clamp(min=1e-7)
                            theta = x_time_new / sqrtK
                            epsilon = torch.clamp(theta - 1.0, min=1e-6)
                            safe_arccosh = torch.where(
                                theta < 1.01,
                                torch.sqrt(2.0 * epsilon),
                                torch.arccosh(torch.clamp(theta, min=1.0 + 1e-7))
                            )
                            x = sqrtK * safe_arccosh * x_space / x_space_norm

                        x = x.to(result.dtype)
                        x = x * scaling
                        result += x

                    elif lora_type == 'hyplora_simplified':

                        k = self.lora_k[active_adapter]
                        x = dropout(x)
                        
                        x = self.hyplora_simplified_inverse(x, k)[..., 1:]
                        
                        x = lora_A(x)
                        x = lora_B(x)  
                        x = torch.cat([((x ** 2).sum(dim=-1, keepdim=True) + k).sqrt(), x], dim=-1)
                        
                        x = self.hyplora_simplified(x, k)
...
class Embedding(nn.Module, LoraLayer):
    ...
    def update_layer(
        self, adapter_name, r, lora_alpha, lora_dropout, lora_type, init_lora_weights, use_rslora, use_dora, lora_bias
    ):
        # collect the kwargs
        kwargs = locals().copy()
        del kwargs["self"]
...
class MultiheadAttention(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._active_adapter = adapter_name
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, lora_type, init_lora_weights, use_rslora)
