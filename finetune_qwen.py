...
def train(
    ...
    def tokenize(prompt, add_eos_token=True):
        ...
        if (add_eos_token and len(result["input_ids"]) < cutoff_len and result["input_ids"][-1] != tokenizer.eos_token_id):
...
def generate_prompt(data_point):
    ...
    if data_point.get("input"):
        user_msg = f"{data_point['instruction'].strip()}\n\nInput:\n{data_point['input'].strip()}"
    else:
        user_msg = data_point["instruction"].strip()

    prompt = (
        f"<|im_start|>system\n{system_prompt}<|im_end|>\n"
        f"<|im_start|>user\n{user_msg}<|im_end|>\n"
        f"<|im_start|>assistant\n{data_point['output'].strip()}<|im_end|>\n"
    )
