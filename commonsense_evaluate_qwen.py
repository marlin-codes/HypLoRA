...
from transformers import GenerationConfig, LlamaForCausalLM, LlamaTokenizer, AutoModelForCausalLM, AutoTokenizer, Gemma3ForConditionalGeneration
...
def main(
    ...
    # save_file = f'experiment/{args.model}-{args.adapter}-{args.dataset}-{args.rank}.json'
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=["boolq", "piqa", "social_i_qa", "hellaswag", "winogrande", "ARC-Challenge", "ARC-Easy", "openbookqa"],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', "LLaMA-13B",'BLOOM-7B', 'GPT-j-6B', 'Qwen2.5-7B-Instruct-1M', 'gemma-3-1b-it', 'gemma-3-4b-it'], required=True)
...
def load_model(args) -> tuple:
    ...
    # )
    if device == "cuda":
        if args.model == "gemma-3-1b-it" or args.model == "gemma-3-4b-it":
